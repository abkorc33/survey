package com.project.survey.common.util;
...
//-----------------------------------------------------------------------------------------------------------
// public class PageMaker ( 상품들의 총 개수, 한 페이지에 보여줄 상품의 개수, 현재 페이지 번호, 한 화면에 보여줄 페이지 번호의 개수 )
//-----------------------------------------------------------------------------------------------------------
public class PageMaker {
    ...
    public void setTotalCount(int totalCount) {
        ...
        // 총 데이터 건수를 가지고 페이지를 계산한다.
        calculatePages();
    }

    //-----------------------------------------------------------------------------------------------------------
    // 페이지 계산		: 총 데이터 건수를 가지고 페이지를 계산한다.
    // Math.ceil(숫자) 	: 입력받은 숫자보다 크거나 같은 정수 중에서 가장 작은 정수를 반환한다.
    // cri.getPage()	: 현재 보고자 화면 하단의 페이지번호를 클릭한 페이지 번호
    ...
    private void calculatePages() {
        ...
        // 시작 페이지는 (endPage - displayPageNum) + 1이 된다.
        // (endPage - displayPageNum) + 1 => (보여줄 마지막 페이지 - 보여줄 페이지 개수) + 1
        startPage = (endPage - displayPageNum) + 1;

        // 마지막 page 번호가 [10]보다 작으면, 시작 page는 무조건 1page로 한다.
        if(startPage <= 0) startPage = 1;	// 시작 페이지는 0이하가 나오면 1로 만든다.

        // tempEndPage : 실제 가지고 있는 페이지의 개수를 구해서 저장하는 변수
        ...
        // 화면 하단에 나타나는 페이지의 개수를 넘어 가므로 [next]버튼을 보여준다.
